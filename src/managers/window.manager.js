...
class WindowManager {
  ...
  async initializeWindows() {
    ...
    try {
      ...
      // chat + llmResponse are consolidated into the single-window hub (main).
  ...
  resizable: true,
    // Allow small min width so it can collapse to one icon, and a generous max so
    // the user can drag the chat hub larger (the collapsed bar stays compact via CSS).
    minWidth: 60,
    maxWidth: 1600,
        minimizable: false,
        maximizable: false,
    ...
    } else if (type === 'chat' || type === 'transcript') {
  ...
  moveBoundWindows(deltaX, deltaY) {
    // The hub is now a single window (llmResponse/chat were consolidated), so we
    // just move the main window. (The old guard required an llmResponse window that
    // no longer exists, which is why ⌘+arrows did nothing.)
    const mainWindow = this.windows.get('main');
  ...
  handleScreenSharingStarted() {
    // Private mode: do NOT hide the overlay when sharing. All windows use
    // setContentProtection(true), so they're invisible to the screen capture /
    // Meet share already — but the user still needs them on their own screen.
    logger.info('Screen sharing detected - keeping content-protected overlay visible to the user');
  }
  ...
  showAllWindows() {
    ...
    // Only restore the hub (main). Settings is shown ONLY when explicitly opened
    // (it's mutually exclusive with the hub) — otherwise toggling visibility would
    // pop Settings on top of the hub and they'd overlap/flicker.
    this.windows.forEach((window, type) => {
      if (type === 'main') {
        this.showOnCurrentDesktop(window);
      }
  ...
  /**
   * Toggle "undetectability" (content protection). When ON, the overlay is
   * invisible to screen capture / sharing (the eye-slash state). Returns the
   * new undetectable boolean. Defaults to undetectable (protected) on launch.
   */
  toggleContentProtection() {
    if (this.contentProtected === undefined) this.contentProtected = true;
    this.contentProtected = !this.contentProtected;
    this.windows.forEach((window) => {
      if (!window.isDestroyed()) {
        try { window.setContentProtection(this.contentProtected); } catch (_) {}
      }
    });
    logger.info('Content protection toggled', { undetectable: this.contentProtected });
  ...
  showLLMResponse(content, metadata = {}) {
    ...
    // Private mode: the overlay windows use setContentProtection(true), so they
    // are already invisible to screen capture / Meet sharing. We intentionally
    // do NOT hide responses while sharing — the user still needs to see them.
    if (this.isScreenBeingShared) {
      logger.debug('Screen sharing active — showing LLM response anyway (content-protected overlay)');
    }
  ...
  showLLMLoading() {
    // Private mode: overlay is content-protected (invisible to screen share),
    // so we keep showing the loading state even while sharing.
  ...
  showSettings() {
    ...
    if (settingsWindow) {
      // Settings and the hub are mutually exclusive: hide the hub (main) while
      // settings is open so they never compete for z-order / flicker.
      const main = this.windows.get('main');
  ...
  hideSettings() {
    ...
    // Restore the hub when settings closes.
    const main = this.windows.get('main');
    if (main && !main.isDestroyed()) this.showOnCurrentDesktop(main);
  }

  // Toggle Settings with a single shortcut: open it (hiding the hub) or, if it's
  // already open, close it (restoring the hub).
  toggleSettings() {
    const s = this.windows.get('settings');
  ...
  // Live transcription window (created lazily, the only extra window besides settings).
  // It MUST be registered in this.windows so (a) we only ever keep one instance, and
  // (b) broadcastToAllWindows() reaches it with the live transcription events.
  async toggleTranscriptWindow() {
    let w = this.windows.get('transcript');
    if (w && !w.isDestroyed()) {
      if (w.isVisible()) { w.hide(); return false; }
      this.showOnCurrentDesktop(w);
      return true;
    }
    w = await this.createWindow('transcript');
    this.windows.set('transcript', w);
    w.on('closed', () => this.windows.delete('transcript'));
    this.showOnCurrentDesktop(w);
    return true;
  }

  closeTranscriptWindow() {
    const w = this.windows.get('transcript');
