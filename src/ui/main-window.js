...
class MainWindowUI {
    ...
    resizeWindowToContent() {
        ...
        setTimeout(() => {
            const commandTab = document.querySelector('.echo-hub');
    ...
    this.shortcutsPopover = document.getElementById('shortcutsPopover');
        ...
        // Screenshot button (the image/screen-context icon).
        this.screenshotButton = document.getElementById('screenshotButton')
            || (document.querySelectorAll('.command-item') || [])[0];

        // Echo bar elements (Cluely-style): ask input + send + settings/eye/history.
        this.askInput = document.getElementById('askInput');
        this.askSend = document.getElementById('askSend');
        this.settingsButton = document.getElementById('settingsButton');
        this.eyeButton = document.getElementById('eyeButton');
        this.historyButton = document.getElementById('historyButton');
        this.micInButton = document.getElementById('micInButton');
        this.audioOutButton = document.getElementById('audioOutButton');

        // Pause/resume audio per channel: 'me' = my mic (input), 'them' = system audio (output).
        const wireAudioToggle = (el, channel) => {
            if (!el) return;
            el.addEventListener('click', () => {
                if (!(window.electronAPI && window.electronAPI.toggleAudioChannel)) return;
                window.electronAPI.toggleAudioChannel(channel).then((state) => {
                    const paused = channel === 'them' ? state.them : state.me;
                    el.classList.toggle('paused', !!paused);
                    el.classList.toggle('mic-on', !paused);
                }).catch(() => {});
            });
        };
        wireAudioToggle(this.micInButton, 'me');
        wireAudioToggle(this.audioOutButton, 'them');

        // Wire the "Ask or search anything" input: send to the brain and open the chat.
        const submitAsk = () => {
            if (!this.askInput) return;
            const text = (this.askInput.value || '').trim();
            // Open the chat IN-WINDOW (single-window hub), no separate window.
            if (window.Hub) window.Hub.showChat();
            if (!text) return;
            this.askInput.value = '';
            if (window.electronAPI && window.electronAPI.sendChatMessage) window.electronAPI.sendChatMessage(text);
        };
        if (this.askInput) {
        ...
        if (this.askSend) this.askSend.addEventListener('click', submitAsk);
        if (this.settingsButton) this.settingsButton.addEventListener('click', () => {
            if (window.electronAPI && window.electronAPI.showSettings) window.electronAPI.showSettings();
        });
        if (this.historyButton) this.historyButton.addEventListener('click', () => {
            if (window.Hub) window.Hub.toggleHistory();
        });
        this.transcriptButton = document.getElementById('transcriptButton');
        if (this.transcriptButton) this.transcriptButton.addEventListener('click', () => {
            // Live transcript is now an inline hub section (like chat), not a window.
            if (window.Hub) window.Hub.toggleTranscript();
        });
        // Down arrow opens History in-window (no system arrow hijack).
        document.addEventListener('keydown', (e) => {
            if (e.key === 'ArrowDown' && this.isInteractive && window.Hub) {
                e.preventDefault();
                window.Hub.toggleHistory();
            }
            // Escape collapses the hub back to just the bar.
            if (e.key === 'Escape' && window.Hub) window.Hub.collapse();
        });
        // Eye = detectable (others can see Echo on screen share); incognito person =
        // undetectable (hidden from screen share / recording).
        const setEyeIcon = (undetectable) => {
            if (!this.eyeButton) return;
            this.eyeButton.classList.toggle('active', !!undetectable);
            const icon = this.eyeButton.querySelector('i');
            if (icon) icon.className = undetectable ? 'fas fa-user-secret' : 'fas fa-eye';
            this.eyeButton.title = undetectable ? 'Indetectable (oculto en compartir pantalla)' : 'Detectable';
        };
        if (this.eyeButton) this.eyeButton.addEventListener('click', () => {
            if (window.electronAPI && window.electronAPI.toggleUndetectability) {
                window.electronAPI.toggleUndetectability().then(setEyeIcon).catch(() => {});
            }
        });
        // Initialize the eye icon from the persisted setting.
        if (window.electronAPI && window.electronAPI.getSettings) {
            window.electronAPI.getSettings().then((s) => { if (s) setEyeIcon(!!s.undetectable); }).catch(() => {});
        }

        if (!this.statusDot || !this.micButton || !this.screenshotButton) {
            console.warn('Some Echo bar elements were not found; continuing with what is available');
        }

        // Screenshot click handler
        if (this.screenshotButton) this.screenshotButton.addEventListener('click', () => {
    ...
    showSettingsMenu() {
        ...
        const quitOption = this.createMenuItem('Quit Echo', 'fa-power-off', () => {
